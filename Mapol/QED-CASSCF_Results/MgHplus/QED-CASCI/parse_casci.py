...
def read_casci_data(filename):
    """
    Reads CASCI data from a file with headers:
    #Bond_Length    E0          E1          E2
    and subsequent columns of data.

    Args:
        filename (str): Path to the CASCI output file.
...
def format_data_for_qcarchive(casci_data):
    """
    Formats the raw CASCI data into a dictionary structure
    resembling QCArchive standards (where applicable).

    Args:
        casci_data (dict): Dictionary containing the raw CASCI data.
    ...
    formatted_data = {
        "provenance": {
            "creator": "QED-CASCI",  # Replace with your code's name
            "version": "1.0",       # Replace with your code's version
            "routine": "qed_casci_energy_scan" # Changed routine name
        },
        "input_data": {
            "bond_lengths": casci_data.get("bond_lengths"),
            "units": "angstrom",
        },
        "properties": {
            "energies": casci_data.get("energies"),
            "units": "hartree"
...
if __name__ == "__main__":
    casci_output_file = "energies_0_05.dat"  # Replace with your DMRG output filename
    json_output_file = "mghplus_casci_ccpVDZ_lz__10p_05_lambda_0p05_om_0.136.json"

    raw_data = read_casci_data(casci_output_file)
